function [In_max_mean,In_max_std,f1_all]=studies_or()

A=load('measurements_or_set_1.m');
do_age=load('do_ages_or_set_1.m');
interp_data=load('interp_dat_or_set_1.m');
...
prob_min=zeros(382,1);
prob_max=zeros(382,1);
prob=zeros(382,1);

for i=1:length(A)/2
    len(1,i)=nnz(A(2:14,2*i))+1;
...
success_and_predicted_success=zeros(382,length(thre_1));
failure_but_predicted_success=zeros(382,length(thre_1));
success_but_predicted_failure=zeros(382,length(thre_1));
failure_and_predicted_failure=zeros(382,length(thre_1));
GFR_critical=20;
...
%plot(thr,In_min,thr,In,thr,In_max)

%legend('GFR min', 'GFR mean', 'GFR max')
